...
def evaluate_multinomial_NB(dataset_name):
    ...
    try:
        spam_email_bag_of_words, ham_email_bag_of_words, text_in_all_document, spam_mail_in_all_documents, ham_mail_in_all_documents, size_of_total_dataset, size_of_spam_dataset, size_of_ham_dataset, total_file_dictionary = bagofWords.convert_to_bag_of_words(
            dataset_name, True)
    except:
        print("You have given wrong file name, please check and run again")
        exit(-1)
    prior, conditional_probability, conditional_probability_of_non_occurring_word = multinomialBayes.train_multinomial_NB(
    ...
    for each_document in spam_email_bag_of_words:
        spam_predict.append(multinomialBayes.test_multinomial_naive_bayes(prior, conditional_probability,
    ...
    for each_document in ham_email_bag_of_words:
        ham_predict.append(multinomialBayes.test_multinomial_naive_bayes(prior, conditional_probability,
                                                                                 conditional_probability_of_non_occurring_word,
                                                                                 each_document))
